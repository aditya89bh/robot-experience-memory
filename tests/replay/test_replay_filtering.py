...
def test_replay_filters_by_success() -> None:
    ...
    events = ReplayEngine(store).replay(filters=ExperienceFilter(success=False))
...
def test_replay_filters_by_robot_id() -> None:
    ...
    events = ReplayEngine(store).replay(filters=ExperienceFilter(robot_id="robot-b"))
...
def test_replay_filters_by_action_type() -> None:
    ...
    events = ReplayEngine(store).replay(filters=ExperienceFilter(action_type="grasp"))
...
def test_replay_applies_pagination() -> None:
    ...
    events = ReplayEngine(store).replay(pagination=Pagination(offset=1, limit=1))
